...
dump_virtual_tree(struct Page *node, int class) {
    ...
    if ((node->state & NODE_TYPE_MASK) == MAPPING_NODE) {
        uintptr_t pstart = node->phy->addr;
        uintptr_t pend = pstart + CLASS_SIZE(node->phy->class);
        cprintf("+ node (class %d) [%08lx, %08lx)\n", node->phy->class, pstart, pend);
    } else {
        cprintf("+ node (intermediate)\n");
    }
...
dump_page_table(pte_t *pml4) {
    cprintf("Page table:\n");
    for (size_t pml4i = 0; pml4i < PT_ENTRY_COUNT; ++pml4i) {
        pml4e_t pml4e = pml4[pml4i];

        if (!(pml4e & PTE_P)) continue;

        dump_entry(pml4e, 0, false);

        pdpe_t* pdp = KADDR(PTE_ADDR(pml4e));

        for (size_t pdpi = 0; pdpi < PT_ENTRY_COUNT; ++pdpi) {
            pdpe_t pdpe = pdp[pdpi];

            if (!(pdpe & PTE_P)) continue;

            bool isLeafPdpe = (pdpe & PTE_PS);

            dump_entry(pdpe, 1 * GB, isLeafPdpe);

            if (!isLeafPdpe) {
                pde_t* pd = KADDR(PTE_ADDR(pdpe));

                for (size_t pdi = 0; pdi < PT_ENTRY_COUNT; ++pdi) {
                    pde_t pde = pd[pdi];

                    if (!(pde & PTE_P)) continue;

                    bool isLeafPde = (pde & PTE_PS);

                    dump_entry(pde, 2 * MB, isLeafPde);

                    if (!isLeafPde) {
                        pte_t* pt = KADDR(PTE_ADDR(pde));

                        for (size_t pi = 0; pi < PT_ENTRY_COUNT; ++pi) {
                            pte_t pe = pt[pi];
                            if (!(pe & PTE_P)) continue;
                            dump_entry(pe, 4 * KB, true);
                        }
                    }
                }
            }
...
unmap_page(struct AddressSpace *spc, uintptr_t addr, int class) {
    ...
    if (!(pd[pdi0] & PTE_P))
        return;
    /* otherwise we need to split 2*MB hw page
...
init_memory(void) {
    ...
    for (; mstart < mend; mstart = (void *)((uint8_t *)mstart + uefi_lp->MemoryMapDescriptorSize)) {
        if (mstart->Attribute & EFI_MEMORY_RUNTIME) {
            ...
            res = map_physical_region(&kspace, mstart->VirtualStart, mstart->PhysicalStart, mstart->NumberOfPages * EFI_PAGE_SIZE, PROT_R | PROT_W);
            assert(!res);
        }
    }
    ...
    map_physical_region(&kspace, FRAMEBUFFER, uefi_lp->FrameBufferBase, uefi_lp->FrameBufferSize, PROT_R | PROT_W | PROT_WC);
    assert(!res);
    map_physical_region(&kspace, X86ADDR(KERN_BASE_ADDR), 0, MIN(MAX_LOW_ADDR_KERN_SIZE, max_memory_map_addr), PROT_R | PROT_W | ALLOC_WEAK);
    assert(!res);
    map_physical_region(&kspace, X86ADDR((uintptr_t)__text_start), PADDR(__text_start), ROUNDUP((uintptr_t)__text_end, CLASS_SIZE(0)) - (uintptr_t)__text_start, PROT_R | PROT_X);
    assert(!res);
    map_physical_region(&kspace, X86ADDR(KERN_STACK_TOP - KERN_STACK_SIZE), PADDR(bootstack), KERN_STACK_SIZE, PROT_R | PROT_W);
    assert(!res);
    map_physical_region(&kspace, X86ADDR(KERN_PF_STACK_TOP - KERN_PF_STACK_SIZE), PADDR(pfstack), KERN_PF_STACK_SIZE, PROT_R | PROT_W);
    assert(!res);
